    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li class="active">Employee Current Status</li>
            </ol>
        </section>
                                    ...
                                    <div style="margin-top:20px;" class="col-xs-12 col-sm-12 col-md-12">
                                        ...
                                        <table class="table table-bordered table-striped" width="100%" border="1" cellspacing="2" cellpadding="2">
                                            <thead>
                                                ...
                                                <?php
                                                    // print_r($get_all_employees_list);
                                                   foreach ($get_all_employees_list as $employee) {
                                                    // //    print_r($employee->empStatus);
                                                    // $classname = $employee->empStatus == 'Active'? 'label-success' : ($employee->empStatus == 'Break' ? 'label-warning' : 'label-danger');
                                                ?>
                                                    <tr>
                                                        <td>

                                                        <?php if ($employee->empStatus == 'Active') { ?>
                                                            <span class="label label-success"><?php echo $employee->empStatus; ?></span>
                                                        <?php } ?>
                                                        <?php if ($employee->empStatus == 'Break') { ?>
                                                            <span class="label label-success">Active</span>
                                                            <span class="label label-warning">On break</span>
                                                        <?php } ?>
                                                        <?php if ($employee->empStatus == 'Over-time') { ?>
                                                                <span class="label label-success">Active</span>
                                                                <span class="label label-danger">Over-time</span>
                                                        <?php } ?>
                                                        <?php if ($employee->empStatus == 'Inactive') { ?>
                                                            <span class="label label-danger">Inactive</span>
                                                        <?php } ?>
                                                            
                                                        </td>
                                                        <td><?php echo $employee->first_name." ".$employee->last_name; ?></td>
                                                        <td><a href="<?php echo site_url('addEdit-employee-shifthours?employee_id='.$employee->employee_id); ?>"><span class="glyphicon glyphicon-pencil"></span></a></td>
                                                    </tr>
                                                <?php 
                                                    // $i++;
                                                    } 
