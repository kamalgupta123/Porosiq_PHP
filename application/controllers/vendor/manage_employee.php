...
//        $this->load->view('vendor/employees_project_timesheet', $data);
    ...
    public function open_requirements() {
        ...
        if (LATAM) {
            $data['requisition_list'] = $this->employee_model->getRequisitionList($vendor_id);
        }
...
//        die;
    ...
    public function edit_vendor_employee_work_order() {
        ...
        if (LATAM) {
            ...
            $data['get_vendor_details'] = $this->employee_model->getVendorDetails($vendor_id);
            $data['get_employee_details'] = $this->employee_model->getEmployeeListsByVendor($vendor_id);
            $data['get_admin_details'] = $this->employee_model->getAdminDetails($admin_id);
            $data['get_work_details'] = $this->employee_model->getWorkDetailsByEmp($employee_id);
//print_r($data['get_work_details']);
        }
        else {
    ...
    public function update_vendor_employee_work_order() {
        if (LATAM) {
            ...
            $admin_signature = $get_admin_details[0]['first_name'] . " " . $get_admin_details[0]['last_name'];
            ...
            if (isset($vendor_signature) && $vendor_signature == '') {
                $this->session->set_flashdata('error_msg', 'Vendor signature field cannot be blank');
                redirect(base_url('edit_vendor_employee_work_order/' . base64_encode($employee_id)));
                ...
                if ($update_query != '') {
                    ...
                    $msg = $admin_name . ' successfully signed work order.';
                    $data['msg'] = $msg;
                    //Load email library
                    $this->load->library('email');
                    ...
                    foreach ($recipients as $rtval) {
                        ...
                        $insert_arr = array(
                            ...
                            "sender_id" => $vendor_id,
                            "sender_type" => "vendor",
                            "subject" => "Work Order Signed Successfully",
                            "message" => $msg,
                            "entry_date" => date("Y-m-d h:i:s"),
                            "is_deleted" => '0',
                            "status" => '0'
...
//            die;
    ...
    public function vendor_employee_payment() {
        ...
        $vendor_id = $data['get_details'][0]['vendor_id'];
        $data['get_vendor'] = $this->vendor_model->getVendorLists();
        $data['get_invoice_list'] = $this->profile_model->getInvoiceList();
