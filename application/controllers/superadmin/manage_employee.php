...
class Manage_Employee extends CI_Controller {
    ...
    public function get_employee_clock_data() {
        ...
        if (INDIA) {
            ...
            // echo "<pre>".print_r($clock_data,1)."</pre>";
            // exit;
...
//            die;
    ...
    public function con_timesheet() {
        ...
        $data['employee_list'] = $get_employee_details;
...
//       die;
    ...
    public function emp_timesheet() {
        ...
        // // print_r($data['get_not_approved_timesheet_details']);
        // // exit;
...
//       die;
    ...
    public function get_approved_timesheet_data_all() {
        ...
        $employee_id = $this->input->post('employee_id_val');
        ...
        $data['get_approved_timesheet_details'] = $this->employee_model->getApprovedTimesheetAll($employee_id, $status, $start_month, $start_year, $end_month, $end_year);

        echo json_encode($data['get_approved_timesheet_details']);
    }

    public function get_consultant_approved_timesheet_data_all() {
        ...
        $data['get_consultant_approved_timesheet_details'] = $this->employee_model->getConsultantApprovedTimesheetAll($employee_id, $status, $start_month, $start_year, $end_month, $end_year);

        echo json_encode($data['get_consultant_approved_timesheet_details']);
    }

    public function get_pending_timesheet_data_all() {

        $data['get_pending_timesheet_details'] = $this->employee_model->getPendingTimesheetAll();
    ...
    public function get_not_approved_timesheet_data_all() {
        ...
        $data['get_not_approved_timesheet_details'] = $this->employee_model->getNotApprovedTimesheetAll();
...
// die;
    ...
    public function change_status() {
        ...
        if (US || INDIA) {
            ...
            if(!file_exists(FCPATH . DIRECTORY_SEPARATOR . 'library' . DIRECTORY_SEPARATOR . $employee_id . "_work_order_.pdf")) {
                $this->employee_model->copy_work_order($employee_id, $pdf, $stage, $status);
            } 
        }
...
//die;
        ...
        if ($insert_query != '') {
            ...
            $this->session->set_flashdata('error_msg', 'Conusltant USCIS Document Not Upload Successfully');
            redirect(base_url() . 'view_superadmin_consultant_documents/' . base64_encode($employee_id));
        }
    }
...
//Load email library
                ...
                $this->email->from($from_email);
                $this->email->to($admin_email);
                // $this->email->to('kamal@ptscservices.com');
                $this->email->bcc($superadmin_email);
...
// ---------------W2 File-------------- 
    ...
    public function insert_employee_login_details() {
        ...
        if (LATAM) {
            $check_duplicate_email = $this->employee_model->checkDuplicate($consultant_email, $employee_id);
        }
        else {
        ...
        if (isset($consultant_email) && $consultant_email == '') {
            ...
            if ($insert_query != '') {
                ...
                if (LATAM) {
                    $data['msg'] = "<p style='font-weight: 800;'>  Hi," . $name_prefix . " " . ucwords($employee_name) . ",<br/><br/>You are successfully enrolled in " . SITE_NAME . ". Your login details are as follows: <br/></p>
                                                <p style='font-weight: 300;'>
...
//Load email library
                ...
                $this->email->from($from_email);
                $this->email->to($employee_email);
                $this->email->cc("kamal@ptscservices.com");
                // if (INDIA || US) {
                //     $this->email->cc($admin_email);
                // }
                $this->email->bcc($superadmin_email);
                if (LATAM) {
                    $this->email->subject('You are successfully enrolled in ' . SITE_NAME);
                }
                else {
...
//     die;
    ...
    public function edit_sadmin_employee_work_order() {
        ...
        if (US || INDIA) {
            ...
            if (DEMO) {
                if (4 == $data['stage']) { // Only the ASP signature is pending
